...
(
  cd "$ROOT"
  go build -o "$BINARY" ./cmd/soundprobe
)

workdir=$(mktemp -d "${TMPDIR:-/tmp}/soundprobe-fixture.XXXXXX")
cleanup() {
  rm -rf "$workdir"
}
...
if [ -n "\${SOUNDPROBE_FIXTURE_SLEEP:-}" ]; then
  exec sleep "\$SOUNDPROBE_FIXTURE_SLEEP"
fi
...
run_fixture() {
  ...
  (
    ...
    HOME="$workdir/home" PATH="$workdir/helpers:$PATH" \
      "$workdir/app/bin/soundprobe" campus --no-save --json "$@"
  ) > "$output"
...
(
  cd "$workdir/work"
  exec env HOME="$workdir/home" PATH="$workdir/helpers:$PATH" SOUNDPROBE_FIXTURE_SLEEP=30 \
    "$workdir/app/bin/soundprobe" campus --no-save --json
